...
import type {
  ...
  GuideProfile as GuideProfileType,
  TouristProfile as TouristProfileType,
} from "@/types/profile";
...
export default async function ProfilePage() {
  ...
  if (userInfo.role === "GUIDE") {
    ...
    // Calculate reviews and average rating from all listings - fetch in parallel for better performance
    const reviewPromises = listings.map((listing: any) =>
      getReviews({ listingId: listing.id, page: 1, limit: 100 })
    );
    const reviewResults = await Promise.all(reviewPromises);

    let allReviews: any[] = [];
    for (const listingReviewsResult of reviewResults) {
    ...
    const badges = (profileData.badges || []).map(
