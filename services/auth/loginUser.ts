...
export const loginUser = async (
  ...
  try {
    ...
    if (!result.success) {
      ...
      const isUnverifiedError =
        ...
        result.errorMessages?.some(
          (err: any) => err.message === "User is not verified"
        );
      ...
      if (isUnverifiedError) {
        ...
        redirect(
          `/verify-otp?email=${encodeURIComponent(email as string)}&name=User`
        );
      }
    ...
    if (
      errorMessage.includes("secret or public key must be provided") ||
      errorMessage.includes("JWT_ACCESS_SECRET")
    ) {
      return {
        success: false,
        message:
          "JWT_ACCESS_SECRET is not configured. Please add JWT_ACCESS_SECRET to your .env.local file. It must match the backend's JWT_ACCESS_SECRET.",
        errors: [
          {
            field: "config",
            message: "JWT_ACCESS_SECRET environment variable is missing",
          },
        ],
      };
    }
